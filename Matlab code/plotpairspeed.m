...
figure_speed = figure('Name','speed');
axes1 = axes('Parent',figure_speed,'FontSize',16);
plotbymode(leader_data(:,time_column),...
    leader_data(:,speed_column), 'o', ColorSet(1,:),...
    leader_data(:,mode_column),...
    leader_data(:,type_column), 0);
hold on;
%also plot its leader
...
if(row>0)
    ...
    plotbymode(leader_leader_data(:,time_column),...
        leader_leader_data(:,speed_column), 's', ColorSet(1,:),...
        leader_leader_data(:,mode_column),...
        -1, 0); %-1 to plot cutter's leader
    hold on;
end
index=2;
for i=1:plot_sample_num
    ...
    if row==0
        ...
        plotbymode(temp_follow(:,time_column),...
            temp_follow(:,speed_column), marker,...
            ColorSet(index,:),...
            temp_follow(:,mode_column),...
            temp_follow(:,type_column), 1);
        index = index + 1;
        hold on;
